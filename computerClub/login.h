...
namespace computerClub {
	...
	{
	public:
		login(Theme* theme, Lang* lang, Save* save)
		{
			InitializeComponent();
			this->theme = theme;
			this->lang = lang;
	...
	protected:
		...
		void set_theme()
		{
			...
			if (save->Lang() != "Save\\rus_Rus.txt")
			{
				...
				this->label1->Text = Conv::ToSystemString(lang->autorez);
			}
			else
			{
				SetRus();
		...
		void SetRus()
		{
			this->button1->Text = % String("авторизаци€");
		...
		while (reader->Read())
			if (reader->GetString(1) == textBox1->Text)
			{
				...
				if (save->Lang() != "Save\\rus_Rus.txt")
					...
					MessageBox::Show("”спешно авторизирован");
				this->Close();
				return;
			}
		reader->Close();
